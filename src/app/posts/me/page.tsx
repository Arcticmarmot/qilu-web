  ...
  return (
    <article
      className={cx(
        ...
        blockHeights[index % blockHeights.length],
      )}
    >
      <Link href={`/posts/me/${post.id}`} className="block">
        <div className={cx("bg-gradient-to-br", bannerHeights[(post.id + index) % bannerHeights.length], palette[index % palette.length])}>
          <div className="flex h-full items-end justify-end bg-background/10 p-3">
            <span
              className={cx(
                "rounded-md bg-background/72 px-3 py-1 text-xs backdrop-blur",
                post.visibility === 2 ? "text-accent-strong" : "text-accent",
              )}
            >
      ...
      <div className="flex flex-1 flex-col p-4">
        <Link href={`/posts/me/${post.id}`} className="block">
          ...
          <p className={cx("mt-2 break-words text-sm leading-6 text-muted", previewLines[(post.id + index) % previewLines.length])}>
            {preview}
          </p>
        </Link>
...
function MyPostsContent() {
  ...
  const handlePageChange = (nextPage: number) => {
    ...
    router.push(`/posts/me?current=${boundedPage}&size=${POST_PAGE_SIZE}`);
  };
